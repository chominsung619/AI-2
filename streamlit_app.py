...
# Google Drive 파일 ID
file_id = '1NRi15ZKZs8CuMB6Lb51Gro2rO15ivU8O'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/vq47V7t/image.jpg?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=aPCe-0ENxZw", start_time=0)
